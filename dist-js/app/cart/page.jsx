...
export default function CartPage() {
    ...
    return (<main className="min-h-screen bg-gray-50">
            ...
            <div className="container mx-auto px-4 sm:px-6 py-4 sm:py-6">
                {cartItems.length > 0 ? (<div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
                        {/* Cart Items */}
                        <div className="lg:col-span-2 space-y-4">
                            <div className="bg-white border border-gray-200 rounded-sm">
                                ...
                                {cartItems.map((item) => (<div key={item.id} className="p-4 border-b border-gray-200 last:border-0">
                                        <div className="flex gap-4">
                                            {/* Image */}
                                            <Link to={`/shop/${item.id}`} className="flex-shrink-0">
                                                <img src={item.image} alt={item.name} className="w-24 h-24 sm:w-28 sm:h-28 object-cover rounded-sm border border-gray-200"/>
                                            </Link>
                                            ...
                                            <div className="flex-1 min-w-0">
                                                ...
                                                <div className="flex items-center gap-2 mb-3 flex-wrap">
                                                    ...
                                                    {('originalPrice' in item && item.originalPrice) ? (<span className="text-sm text-gray-500 line-through">
                                                ...
                                                <p className="text-xs sm:text-sm text-gray-600 mb-3">
                                                    Delivery by <span className="font-semibold">{item.delivery ?? '5-7 days'}</span>
                                                </p>
                            ...
                            <div className="lg:hidden bg-white border border-gray-200 rounded-sm p-4">
                                <button onClick={handlePlaceOrder} className="block w-full bg-orange-600 hover:bg-orange-700 text-white font-semibold py-3 rounded-sm text-center transition-colors">
                                    PLACE ORDER
                                </button>
                            </div>
                        </div>
                        ...
                        <div className="lg:col-span-1">
                            <div className="bg-white border border-gray-200 rounded-sm sticky top-20">
                                ...
                                <div className="p-4 space-y-3">
                                    ...
                                    {deliveryCharges > 0 && (<p className="text-xs text-gray-600 bg-blue-50 p-2 rounded-sm">
                            ...
                            <div className="bg-white border border-gray-200 rounded-sm mt-4 p-4">
                                ...
                                <div className="flex gap-2">
                                    <input type="text" placeholder="Enter coupon code" className="flex-1 px-3 py-2 border border-gray-300 rounded-sm text-sm focus:outline-none focus:ring-1 focus:ring-blue-500"/>
                    ...
                    </div>) : (<div className="bg-white border border-gray-200 rounded-sm p-12 text-center">
                        ...
                        <Link to="/shop" className="inline-block bg-blue-600 text-white px-8 py-3 rounded-sm font-semibold hover:bg-blue-700 transition-colors">
                            Shop Now
                        </Link>
                    </div>)}
            </div>
