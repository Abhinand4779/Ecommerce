        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2">
            <div className="bg-white border border-gray-200 rounded-sm p-6">
              ...
              {items.length === 0 ? (<div className="py-20 text-center">
                  <p className="text-gray-600 mb-4">Your cart is empty.</p>
                  <Link to="/shop" className="inline-block px-5 py-3 bg-blue-600 text-white rounded-sm">Start Shopping</Link>
                </div>) : (<div className="space-y-4">
                  {items.map((it) => (<div key={it.id} className="flex items-center gap-4 border-b border-gray-100 pb-3">
                      ...
                      <div className="text-right">
                        ...
                        {(it.originalPrice) && (<div className="text-xs text-gray-500 line-through">₹{(it.originalPrice).toLocaleString()}</div>)}
                      </div>
                    </div>))}
                </div>)}
            </div>
            ...
            <div className="mt-6 bg-white border border-gray-200 rounded-sm p-6">
              ...
              <div className="mt-6 border-t pt-4">
                <button onClick={handlePlaceOrder} disabled={processing} className="w-full bg-amber-600 hover:bg-amber-700 text-white font-semibold py-3 rounded-sm disabled:opacity-60">
