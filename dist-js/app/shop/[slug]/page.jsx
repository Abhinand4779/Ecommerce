        ...
        <div className="container mx-auto px-4 sm:px-6 py-4 sm:py-6">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 lg:gap-8">
            ...
            <div className="flex flex-col-reverse sm:flex-row gap-4">
              ...
              <div className="flex sm:flex-col gap-2 overflow-x-auto sm:overflow-visible">
                {product.images.map((img, index) => (<button key={index} onClick={() => setSelectedImage(index)} className={`flex-shrink-0 w-16 h-16 sm:w-20 sm:h-20 border-2 rounded-sm overflow-hidden ${selectedImage === index ? 'border-blue-600' : 'border-gray-200'}`}>
            ...
            <div>
            ...
            ].map((offer, i) => (<div key={i} className="flex items-start gap-2 text-sm">
              ...
              <div className="mb-6 pb-6 border-b border-gray-200">
                ...
                <ul className="space-y-2">
                  {product.highlights.map((highlight, i) => (<li key={i} className="flex items-start gap-2 text-sm text-gray-700"><span className="text-gray-400 mt-1">•</span>{highlight}</li>))}
                </ul>
              </div>
              ...
              <div className="mb-6 pb-6 border-b border-gray-200">
                ...
                <div className="grid grid-cols-2 gap-3">
                  {[{ icon: <TruckIcon size={20}/>, text: 'Free Delivery' }, { icon: <ShieldCheck size={20}/>, text: '5 Year Warranty' }, { icon: <RotateCcw size={20}/>, text: '7 Days Return' }, { icon: <Star size={20}/>, text: 'Certified Quality' }].map((service, i) => (<div key={i} className="flex items-center gap-2 text-sm text-gray-700"><span className="text-gray-400">{service.icon}</span>{service.text}</div>))}
                </div>
              </div>
          ...
          <div className="mt-8 bg-white border border-gray-200 rounded-sm p-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {Object.entries(product.specifications).map(([key, value]) => (<div key={key} className="flex py-3 border-b border-gray-200"><span className="w-1/3 text-sm text-gray-600">{key}</span><span className="w-2/3 text-sm text-gray-900 font-medium">{value}</span></div>))}
            </div>
          </div>
