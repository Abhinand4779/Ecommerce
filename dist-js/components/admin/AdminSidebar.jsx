...
const AdminSidebar = () => {
    ...
    return (<aside className="fixed left-0 top-0 h-screen w-64 bg-[#141414] border-r border-white/5 flex flex-col z-50">
            <div className="p-8">
            ...
            <nav className="flex-grow px-4 mt-8">
                <ul className="flex flex-col gap-2">
                    {menuItems.map((item) => (<li key={item.name}>
                            <Link to={item.href} className={`flex items-center gap-4 px-4 py-3 rounded-lg transition-all ${pathname === item.href
