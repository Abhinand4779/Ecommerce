...
const Navbar = () => {
    ...
    return (<header className="bg-white sticky top-0 z-50 shadow-sm border-b border-gray-100">
            {/* Top Bar - Logo and Icons */}
            <div className="container mx-auto px-4 sm:px-6">
                <div className="flex items-center justify-between h-16 sm:h-20">
                    {/* Mobile Menu Button */}
                    <button className="md:hidden p-2 -ml-2 text-gray-600 hover:text-amber-600 transition-colors" onClick={() => setIsMenuOpen(!isMenuOpen)}>
                    ...
                    <div className="hidden md:flex flex-1 max-w-md mx-8">
                        <div className="relative w-full group">
                            <input type="text" placeholder="Search our collection..." className="w-full bg-gray-50 border border-gray-200 rounded-full py-2 pl-10 pr-4 text-sm focus:outline-none focus:ring-1 focus:ring-amber-600 focus:border-amber-600 transition-all"/>
                    ...
                    <div className="flex items-center gap-2 sm:gap-6">
                        ...
                        <button onClick={toggleTheme} aria-label="Toggle theme" title="Toggle theme" className="p-2 text-gray-600 hover:text-amber-600 transition-colors">
                        ...
                        {!user && (<button onClick={onGuestSignIn} className="hidden sm:inline-block px-3 py-1 text-sm text-gray-700 bg-gray-100 rounded hover:bg-gray-200 ml-2">
                                Start as Guest
                            </button>)}

                        <button className="p-2 text-gray-600 hover:text-amber-600 transition-colors relative">
                            <Heart size={22}/>
                            {favorites.length > 0 && (<span className="absolute top-1 right-1 bg-amber-600 text-white text-[10px] font-bold w-4 h-4 rounded-full flex items-center justify-center">{favorites.length}</span>)}
                        </button>

                        <Link to="/cart" className="p-2 text-gray-600 hover:text-amber-600 transition-colors relative">
                            <ShoppingCart size={22}/>
                            {cartCount > 0 && (<span className="absolute top-1 right-1 bg-amber-600 text-white text-[10px] font-bold w-4 h-4 rounded-full flex items-center justify-center">{cartCount}</span>)}
                        </Link>
                    </div>
                </div>
            </div>
            ...
            <nav className="hidden md:block bg-white border-t border-gray-50">
                <div className="container mx-auto px-4">
                    <ul className="flex items-center justify-center gap-10 h-12">
                        {categories.map((cat) => (<li key={cat.name} className="relative" onMouseEnter={() => openDropdownNow(cat.name)} onMouseLeave={closeDropdownDelayed} onFocus={() => openDropdownNow(cat.name)} onBlur={closeDropdownDelayed}>
                                <Link to={cat.href} className="text-sm font-medium text-gray-600 hover:text-amber-600 uppercase tracking-widest transition-colors flex items-center gap-2" aria-haspopup={cat.subcategories ? 'menu' : undefined}>
                                    <span>{cat.name}</span>
                                ...
                                {cat.subcategories && (<div className={`absolute left-0 top-full mt-2 bg-white shadow-lg rounded-md py-2 w-48 transition-all z-50 ${openDropdown === cat.name ? 'opacity-100 translate-y-0 pointer-events-auto' : 'opacity-0 translate-y-2 pointer-events-none'}`} role="menu" aria-label={`${cat.name} submenu`}>
                                        {cat.subcategories.map((sub) => (<Link key={sub.name} to={sub.href} className="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-50">
                                                {sub.name}
                                            </Link>))}
                                    </div>)}
                            </li>))}
            ...
            {isMenuOpen && (<div className="md:hidden fixed inset-0 z-40 bg-white pt-20">
                    <ul className="flex flex-col items-center gap-6 p-4">
                        {categories.map((cat) => (<li key={cat.name} className="w-full text-left">
                                <div className="flex items-center justify-between w-full border-b border-gray-100 py-3">
                                    <Link to={cat.href} className="text-xl font-medium text-gray-800 hover:text-amber-600 block" onClick={() => setIsMenuOpen(false)}>
                                        {cat.name}
                                    </Link>

                                    {cat.subcategories && (<button className="p-2 text-gray-600" onClick={() => setMobileOpen((prev) => ({ ...prev, [cat.name]: !prev[cat.name] }))} aria-expanded={!!mobileOpen[cat.name]}>
                                            {mobileOpen[cat.name] ? <ChevronUp size={18}/> : <ChevronDown size={18}/>}
                                        </button>)}
                                </div>

                                {cat.subcategories && mobileOpen[cat.name] && (<ul className="pl-6 bg-white w-full">
                                        {cat.subcategories.map((sub) => (<li key={sub.name} className="py-2">
                                                <Link to={sub.href} className="text-gray-700 block" onClick={() => setIsMenuOpen(false)}>
                                                    {sub.name}
                                                </Link>
                                            </li>))}
                                    </ul>)}
                ...
                onProfileClick();
                            ...
                            {/* Mobile guest option */}
                            {!user && (<div className="mt-3 px-2">
                                    <button className="w-full btn-secondary" onClick={() => {
                    setIsMenuOpen(false);
                    onGuestSignIn();
                }}>
                                        Start as Guest
