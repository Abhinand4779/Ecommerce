...
export default function ProductDetail({ params }: { params: { id: string } }) {
    ...
    // Try to find product from the shared dataset first
    const found = allProducts.find(p => String(p.id) === params.id || p.id === Number(params.id));
    ...
    const handleAdd = () => {
        try {
            addToCart({ id: Number(product.id), name: product.name, price: product.price, image: product.images?.[selectedImage] ?? product.images?.[0] ?? product.image });
            setAdded(true);
                    ...
                    <div>
                        ...
                        <div className="mb-6 pb-6 border-b border-gray-200">
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                {[
                                    { icon: <TruckIcon size={20} />, text: 'Free Delivery' },
                                    { icon: <ShieldCheck size={20} />, text: '5 Year Warranty' },
                                    { icon: <RotateCcw size={20} />, text: '7 Days Return' },
                                    { icon: <Star size={20} />, text: 'Certified Quality' },
                                ].map((service, i) => (
                        ...
                        <div className="flex gap-3 sticky bottom-0 bg-white py-4 border-t border-gray-200 lg:border-0">
                            ...
                            {added && (
                                <div className="fixed right-4 bottom-6 bg-amber-600 text-white px-4 py-2 rounded shadow-lg">Added to cart</div>
                            )}
                            <button className="flex-1 bg-orange-600 hover:bg-orange-700 text-white font-semibold py-3 rounded-sm flex items-center justify-center gap-2 transition-colors">
                                <Zap size={20} />
                                BUY NOW
                            </button>
                        </div>
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                        {Object.entries(product.specifications).map(([key, value]) => (
                            <div key={key} className="flex py-3 border-b border-gray-200">
                                <span className="w-1/3 text-sm text-gray-600">{key}</span>
